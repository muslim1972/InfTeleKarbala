-- تحديث دالة تعديل الإجازة لتشمل سبب الإلغاء
...
DROP FUNCTION IF EXISTS public.modify_leave_request(uuid, text, date, date, integer, date, text);
...
CREATE OR REPLACE FUNCTION public.modify_leave_request(
    ...
    p_cancellation_reason text DEFAULT NULL::text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    IF v_request.user_id::text != v_user_id::text THEN
         -- سنقوم بإرجاع الأيديات في الرسالة لمعرفة الخلل إذا استمر
    ...
    ELSIF p_modification_type = 'edited' THEN
        ...
        -- إشعار المسؤول الأول بأن الطلب تم تعديله ويحتاج موافقته من جديد
