...
-- 1. Restore Table-Level SELECT to fix PostgREST "permission denied for table"
GRANT SELECT ON TABLE public.profiles TO anon, authenticated;
GRANT SELECT ON TABLE public.summer_training_students TO anon, authenticated;

-- 2. Modify fn_sync_profile_password_to_auth to clear passwords
...
-- 3. Summer students clear trigger
...
-- 4. Scrub existing data
UPDATE public.profiles SET password = NULL, password_hash = NULL, two_factor_code = NULL;
UPDATE public.summer_training_students SET password_hash = NULL;
