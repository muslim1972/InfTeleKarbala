-- TEST RUN (ROLLBACK at end): same as fix-dept-location-sync.sql + rename scenario
-- ASCII only file; Arabic via unicode escapes
-- Pre-checks (incl. tree alignment view)
...
SELECT 'karbala_l3' AS tag, id, name FROM departments WHERE governorate='karbala' AND level=3 ORDER BY id;
SELECT 'babil_ph_before' AS tag, id, name FROM departments WHERE governorate='babil' AND level=3 ORDER BY id;

BEGIN;

-- 1) Fix typo (missing alef) in both tables
UPDATE departments
...
-- TEST SCENARIO: supervisor renames al-hair complex to abu-gharaq (babil)
UPDATE departments
   SET name = E'\u0645\u062C\u0645\u0639 \u0627\u062A\u0635\u0627\u0644\u0627\u062A \u0627\u0628\u0648 \u063A\u0631\u0642'
 WHERE governorate = 'babil'
   AND name LIKE E'\u0645\u062C\u0645\u0639 \u0627\u062A\u0635\u0627\u0644\u0627\u062A \u0627\u0644\u062D\u0631%';

-- Verification
SELECT 'babil_level3_after' AS tag, id, name FROM departments WHERE governorate='babil' AND level=3 ORDER BY id;
SELECT 'babil_locations_after' AS tag, name FROM work_locations WHERE governorate='babil' ORDER BY name;
SELECT 'karbala_locations_untouched' AS tag, name FROM work_locations WHERE governorate='karbala' ORDER BY name;
