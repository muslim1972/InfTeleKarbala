 ...
 *
 * لماذا لا توجد البصرة؟ قياس ميداني (2026): صندوق العشّار يسجّل
 * في OpenStreetMap طرقاً بلا أي مبنى (فجوة تغطية مجتمعية)، وأحياء
 * بصرية أخرى 0–1 مبنى لكل مربع. استُبدلت بمنطقة كربلائية جنوبية
 * موثقة ميدانياً (15 مبنى مغلقاً + 22 طريقاً). هذه القائمة بيد
 * المطور، والمستخدم يضيف مناطقه الخاصة من «قوالب مخصصة» في
 * نافذة الاستيراد — تُحفظ في متصفحه ولا تمس بقية المستخدمين.
 */
export const OSM_PRESETS: OsmPreset[] = [
  ...
  {
    id: 'karbala-south',
    label: 'كربلاء — جنوب المدينة',
    hint: 'حي سكني هادئ — مناسب للمبتدئين',
    bbox: { south: 32.596, west: 44.018, north: 32.599, east: 44.022 },
  },
];
...
function overpassToGeojson(
  ...
  /* منطقة مسجلة كطرق بلا مبانٍ = فجوة تغطية في OSM — رسالة صريحة
     بدل الخطأ المبهم «لا توجد Polygons» من المحوّل */
  if (buildings === 0 && roads > 0) {
    throw new Error(
      'هذه المنطقة مسجلة في OpenStreetMap كطرق بلا مبانٍ (فجوة تغطية) — ' +
        'جرّب منطقة أخرى، أو استورد ملف GeoJSON خاصاً إن توفر لديك'
    );
  }
