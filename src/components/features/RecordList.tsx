    ...
    return (
        <div className="space-y-2">
            {data.map((item: any, idx: number) => (
                <div key={item.id || idx} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 group hover:bg-white/10 transition-colors">
                    <div className="space-y-1">
                        ...
                        {/* Penalty Specifics */}
                        {item.reason && fields[0].key === 'penalty_type' && <p className="text-white/60 text-xs">{item.reason}</p>}
                    </div>
